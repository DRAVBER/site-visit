...
- **Действия** — переключение темы (три состояния: тёмная → светлая → системная), смена языка RU⇄EN, копирование email, печать/PDF-резюме, переходы в GitHub / Telegram / Discord.
...
`/rss.xml` — RSS 2.0-лента со всеми проектами: заголовок, описание (EN), дата последнего коммита как pubDate, категории = категория + теги, permalink-ссылки ведут на deep-link диалога (`/#p=<id>`). Добавление проекта в `data/projects.json` автоматически публикует его в ленту. Кнопка RSS в футере, `<link rel="alternate">` в метаданных — ридеры обнаруживают ленту автоматически. Ссылка на домен — в `src/app/rss.xml/route.ts` (TODO-комментарий, замените при деплое).
